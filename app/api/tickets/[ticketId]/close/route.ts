...
export async function POST(
  ...
  try {
    ...
    // ===============================
    // 1. ปิด ticket (เฉพาะกรณียังไม่ close)
    // ===============================
    const [closeResult]: any = await conn.execute(
      `
      UPDATE tickets
    ...
    // ===============================
    // 2. สร้าง satisfaction ใหม่ทุกครั้ง
    // ===============================
    ...
    // ===============================
    // 5. ส่ง SMS (หลัง commit เท่านั้น)
    // ===============================
    ...
    return NextResponse.json({
      ...
      ticket_id,
      satisfaction_token: token,
      survey_url: `${process.env.NEXT_PUBLIC_FRONTEND_URL}/satisfaction?token=${token}`,
    });
  } catch (err) {
    try {
      await conn.rollback();
